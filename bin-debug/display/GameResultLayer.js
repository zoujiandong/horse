 ...
 *
 */
var GameResultLayer = (function (_super) {
    __extends(GameResultLayer, _super);
    function GameResultLayer() {
        _super.call(this);
        this.init();
    }
    var d = __define,c=GameResultLayer,p=c.prototype;
    p.init = function () {
        var bg = Global.createBitmapByName("resultBG");
        this.addChild(bg);
        ...
        var tw = egret.Tween.get(this.firstHorse, { loop: false });
        tw.to({ scaleX: 1.2, scaleY: 1.2 }, 1000, egret.Ease.sineIn);
        var first = Global.createBitmapByName("first");
        var second = Global.createBitmapByName("second");
        var third = Global.createBitmapByName("third");
        first.x = 240;
        first.y = 49;
        second.x = 80;
        second.y = 70;
...
egret.registerClass(GameResultLayer,'GameResultLayer');
